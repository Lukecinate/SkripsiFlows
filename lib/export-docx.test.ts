...
test("exports a valid docx package", async () => { const result = await exportDocx(document); assert.equal(result.mimeType, "application/vnd.openxmlformats-officedocument.wordprocessingml.document"); assert.ok(result.data.length > 500); assert.equal(String.fromCharCode(...result.data.slice(0, 2)), "PK"); });
