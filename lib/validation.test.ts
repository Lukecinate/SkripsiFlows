...
test("blocks export while low confidence remains", () => { const report = validateDocument(base); assert.equal(report.canExport, false); assert.ok(report.issues.some((issue) => issue.code === "LOW_CONFIDENCE_BLOCK")); });
